...
public class ChatActivity extends AppCompatActivity implements SearchView.OnQueryTextListener{
    ...
    private void MessageFromClient(String FriendEmail,String message,String image){
        ...
        ChatListServer.add(new Chat_Model(data.user.getEmail(), FriendEmail, message, true,new Date()));  //To Store Client Message
        data.user.setChatModelList(new Chat_Model(data.user.getEmail(), FriendEmail, message, true,new Date()));
        if(!image.isEmpty()){
    ...
    public void HandlerChatServer(){
        ...
        if(ch !=null) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ChatListServer.add(new Chat_Model(data.user.getEmail(), friendEmail, (String) list.get(i).getObject(), false,new Date())); //edit the Date
                data.user.setChatModelList(new Chat_Model(data.user.getEmail(), friendEmail, (String) list.get(i).getObject(), false, new Date())); //Edit The Date
            }
